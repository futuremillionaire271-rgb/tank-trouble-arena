// Tank Trouble Online - authoritative game server
...
const TICK = 1000 / 40;        // physics 40 Hz
const SNAP = 1000 / 20;        // snapshots 20 Hz
...
// ---------- maze generation ----------
const CELL = 90, WALL_T = 7;
function generateMaze() {
  const cols = randInt(7, 10), rows = randInt(5, 7);
  ...
  // knock out extra interior walls to create loops (like Tank Trouble)
  const extra = Math.floor(cols * rows * 0.16);
...
function pushCircleOut(obj, r, rect) {
  ...
  if (d === 0) { // center inside rect: push out along smallest axis
...
// bullet bounce: reflect velocity off rect, returns true if hit
...
// raycast a segment against walls -> nearest hit with normal (for laser)
...
// ---------- game constants ----------
const TANK_R = 13, TANK_SPEED = 110, TANK_TURN = 3.6;
const BULLET_R = 4, BULLET_SPEED = 190, BULLET_LIFE = 9, MAX_BULLETS = 5;
const COLORS = ['red', 'green', 'blue'];
const POWERUP_TYPES = ['laser', 'missile', 'gatling', 'frag', 'shield'];
const ROUND_RESET_DELAY = 3000;
...
class Room {
  constructor(code) {
    ...
    this.bullets = [];
    this.powerups = [];
    this.effects = [];   // transient: lasers, explosions
  ...
  addPlayer(ws, name) {
    ...
    const p = {
      ws, name: (name || 'Player').slice(0, 12), color, score: 0,
      x: 0, y: 0, angle: 0, alive: false,
      input: { turn: 0, move: 0, fire: false, vecAngle: null, vecThrottle: 0 },
      weapon: null, ammoSpecial: 0, gatlingCd: 0, fireHeld: false, shield: 0,
    };
    this.players.set(ws, p);
    this.spawnTank(p);
    if (this.players.size >= 2 && !this.roundActive) this.startRound();
    return p;
  }
  removePlayer(ws) {
  ...
  spawnTank(p) {
    // random free cell, far from other tanks
    let best = null;
    for (let i = 0; i < 20; i++) {
    ...
    p.alive = true; p.weapon = null; p.shield = 0;
  }
  startRound() {
    ...
    this.powerups = [];
    this.effects = [];
    this.powerupTimer = rand(3, 5);
    for (const p of this.players.values()) this.spawnTank(p);
    this.broadcast({ type: 'round', maze: this.maze });
  }
  endRoundCheck() {
    ...
    if (alive.length <= 1) {
      this.roundActive = false;
      if (alive.length === 1) { alive[0].score++; this.broadcast({ type: 'roundEnd', winner: alive[0].color }); }
  ...
  killTank(p, killerColor) {
    if (!p.alive) return;
    if (p.shield > 0) { p.shield = 0; this.effects.push({ kind: 'shieldPop', x: p.x, y: p.y, t: 0.4 }); return; }
    p.alive = false;
    this.effects.push({ kind: 'explosion', x: p.x, y: p.y, t: 0.8, color: p.color });
    this.endRoundCheck();
  }

  fireBullet(p, opts = {}) {
    const mine = this.bullets.filter(b => b.owner === p && b.kind === 'normal');
    if (!opts.kind && mine.length >= MAX_BULLETS) return;
    ...
    this.bullets.push({
      ...
      life: opts.life || BULLET_LIFE, r: opts.r || BULLET_R, grace: 0.12, fuse: opts.fuse,
    });
  }
  ...
  fireLaser(p) {
    ...
    for (let bounce = 0; bounce < 6; bounce++) {
      ...
      // tank hits along segment
      for (const o of this.players.values()) {
        if (!o.alive) continue;
        if (o === p && bounce === 0) continue; // can't shoot self point-blank, but bounced beam can
    ...
    this.effects.push({ kind: 'laser', pts, color: p.color, t: 0.35 });
    for (const v of victims) this.killTank(v, p.color);
  }
  ...
  explodeFrag(b) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      this.bullets.push({
        ...
        x: b.x, y: b.y, vx: Math.cos(a) * 160, vy: Math.sin(a) * 160,
        life: 1.1, r: 3, grace: 0.05,
      });
    }
    this.effects.push({ kind: 'explosion', x: b.x, y: b.y, t: 0.6, color: 'orange' });
  }

  handleFire(p) {
    if (!p.alive || !this.roundActive) return;
    if (p.weapon === 'laser') { this.fireLaser(p); p.weapon = null; }
    else if (p.weapon === 'missile') { this.fireBullet(p, { kind: 'missile', speed: 120, life: 8, r: 6 }); p.weapon = null; }
    else if (p.weapon === 'frag') { this.fireBullet(p, { kind: 'frag', speed: 150, life: 5, r: 6, fuse: 1.4 }); p.weapon = null; }
    else if (p.weapon === 'gatling') { /* handled continuously while fire held */ }
    else this.fireBullet(p);
  }
  ...
  update(dt) {
    ...
    for (const p of this.players.values()) {
      ...
      // gatling continuous fire
      if (p.weapon === 'gatling' && p.fireHeld) {
        p.gatlingCd -= dt;
        if (p.gatlingCd <= 0) {
          p.gatlingCd = 0.09;
          this.fireBullet(p, { kind: 'shard', speed: 230, life: 1.6, r: 3, angle: p.angle + rand(-0.09, 0.09) });
          p.ammoSpecial--;
      ...
      for (let i = this.powerups.length - 1; i >= 0; i--) {
        ...
        if (dist2(p.x, p.y, pu.x, pu.y) < (TANK_R + 12) ** 2) {
          ...
          else { p.weapon = pu.type; if (pu.type === 'gatling') { p.ammoSpecial = 25; p.gatlingCd = 0; } }
        }
      }
    }
    // --- bullets ---
    for (let i = this.bullets.length - 1; i >= 0; i--) {
      ...
      if (b.kind === 'missile') {
        ...
        for (const o of this.players.values()) {
          if (!o.alive || (o === b.owner && b.life > 6.5)) continue;
        ...
        if (target) {
          ...
          const na = cur + clamp(diff, -2.6 * dt, 2.6 * dt);
      ...
      for (let s = 0; s < steps && !dead; s++) {
        ...
        for (const w of walls) {
          if (bounceOffRect(b, b.r, w)) {
            if (b.kind === 'missile') { this.effects.push({ kind: 'explosion', x: b.x, y: b.y, t: 0.5, color: 'orange' }); dead = true; }
        ...
        for (const o of this.players.values()) {
          ...
          if (dist2(b.x, b.y, o.x, o.y) < (TANK_R + b.r) ** 2) {
            ...
            if (b.kind === 'missile') this.effects.push({ kind: 'explosion', x: b.x, y: b.y, t: 0.5, color: 'orange' });
            this.killTank(o, b.ownerColor);
            dead = true; break;
          }
        }
      }
  ...
  snapshot() {
    return {
      ...
      tanks: [...this.players.values()].map(p => ({
        c: p.color, n: p.name, s: p.score, x: Math.round(p.x * 10) / 10, y: Math.round(p.y * 10) / 10,
        a: Math.round(p.angle * 1000) / 1000, al: p.alive, w: p.weapon, sh: p.shield > 0,
      })),
      bullets: this.bullets.map(b => ({ x: Math.round(b.x), y: Math.round(b.y), k: b.kind, c: b.ownerColor, r: b.r })),
  ...
  ws.on('message', (msg) => {
    ...
    if (m.type === 'create' || m.type === 'join' || m.type === 'quick') {
      ...
      if (m.type === 'create') { room = new Room(roomCode()); rooms.set(room.code, room); }
        ...
        if (room.players.size >= 3) { ws.send(JSON.stringify({ type: 'error', msg: 'Room is full (3 players max)' })); return; }
      } else { // quick
        room = [...rooms.values()].find(r => r.players.size > 0 && r.players.size < 3);
        if (!room) { room = new Room(roomCode()); rooms.set(room.code, room); }
      }
      const p = room.addPlayer(ws, m.name);
      if (!p) { ws.send(JSON.stringify({ type: 'error', msg: 'Room is full (3 players max)' })); return; }
      ws.room = room; ws.player = p;
      ws.send(JSON.stringify({ type: 'joined', room: room.code, color: p.color, maze: room.maze }));
