...
(async () => {
  ...
  const si = r.data && r.data.serviceInstance;
  console.log('configured region:', si && si.region);
  const meta = si && si.latestDeployment && si.latestDeployment.meta;
  if (meta) {
    const str = JSON.stringify(meta);
    const m = str.match(/"region"\s*:\s*"([^"]+)"/g);
    console.log('regions in deployment meta:', m || 'none found');
  }
})();
