...
(async () => {
  const r = await gql(`query($serviceId: String!, $environmentId: String!) {
    deployments(first: 2, input: { serviceId: $serviceId, environmentId: $environmentId }) {
      edges { node { id status createdAt meta } }
    }
  }`, { serviceId: SERVICE_ID, environmentId: ENV_ID });
  for (const e of r.data.deployments.edges) {
    console.log(e.node.status, e.node.createdAt, JSON.stringify(e.node.meta && e.node.meta.serviceManifest && e.node.meta.serviceManifest.deploy || {}).slice(0, 200));
  }
})();
