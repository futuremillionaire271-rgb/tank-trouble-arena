...
async function main() {
  ...
  const c5 = await mkClient('Eve', { type: 'join', room: roomCode });
  const c6 = await mkClient('Frank', { type: 'join', room: roomCode });
  // 7th should be rejected
  const c7 = await mkClient('Grace', { type: 'join', room: roomCode });
  console.log('colors:', results.colors.join(','), '| 7th rejected:', c7 === null);

  // drive: Alice spins and fires constantly, others idle -> eventually someone dies
  const t0 = Date.now();
  const iv = setInterval(() => {
    for (const [i, c] of [c1, c2, c3, c4, c5, c6].entries()) {
      if (c && c.readyState === 1) c.send(JSON.stringify({ type: 'input', move: 1, turn: (i % 2 ? -1 : 1) * Math.random(), fire: Math.random() > 0.4 }));
    }
  }, 50);
  ...
  const pass = results.joined.length === 6 && c7 === null && results.states > 20 && results.roundEnds >= 1 && results.rounds >= 1;
  console.log(pass ? 'ALL TESTS PASS' : 'TESTS FAILED');
  [c1, c2, c3, c4, c5, c6].forEach(c => c && c.close());
